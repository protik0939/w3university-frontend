...
import { fetchAdminExercises, deleteExercise, AdminExercise } from '@/lib/api/admin'
import AdminSidebar from '@/Components/Admin/AdminSidebar'
import Breadcrumb from '@/Components/Admin/Breadcrumb'
import { Search, Plus, Edit2, Trash2, Eye, Loader2, ChevronLeft, ChevronRight, Code } from 'lucide-react'
...
function ExercisesContent() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      loadExercises() // Reload to update counts
    } catch {
  ...
  const handleStatusChange = (value: 'all' | 'published' | 'draft' | 'archived') => {
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border border-gray-200 dark:border-gray-800 rounded-2xl p-6 mb-6 mt-6 transition-colors">
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-1">Manage Exercises</h1>
          ...
          <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 mb-6 transition-colors">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div className="flex gap-3 w-full md:w-auto">
                <select
                  value={statusFilter}
                  onChange={(e) => handleStatusChange(e.target.value as 'all' | 'published' | 'draft' | 'archived')}
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 transition-colors">
                      <tr>
                        ...
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider transition-colors">Difficulty</th>
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider transition-colors">Language</th>
                        ...
                        <th className="px-6 py-4 text-left text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase tracking-wider transition-colors">Completions</th>
                    ...
                    <tbody className="divide-y divide-gray-200 dark:divide-gray-800 transition-colors">
                      {exercises.map(exercise => (
                        <tr key={exercise.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors">
                          ...
                          <td className="px-6 py-4">
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getDifficultyColor(exercise.difficulty)} transition-colors`}>
                              {exercise.difficulty}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 transition-colors">
                            {exercise.language_name || exercise.programming_language || '-'}
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 transition-colors">
                            <div className="flex items-center gap-1">
                              <Eye size={14} className="text-gray-400" />
                              {exercise.views?.toLocaleString() || 0}
                            </div>
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 transition-colors">
                            {exercise.completions?.toLocaleString() || 0}
                          </td>
                          <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400 transition-colors">
